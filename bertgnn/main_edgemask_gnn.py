...
#         batch_data = batch_data.to(device)
        ...
        edge_rep = node_rep[batch_edgeindex[0][batch_masked_edgeidx]] + node_rep[batch_edgeindex[1][batch_masked_edgeidx]]
...
def main():
    ...
    parser.add_argument('--model_file', type=str, default = './model/', help='filename to output the model')
...
#         print(new_data.masked_edge_idx)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        train_loss, train_acc = train(epoch, args, data_list, model_list, optimizer_list, device)
        print(train_loss, train_acc)
        with open("edgemask/log.txt", "+a") as f:
            f.write(str(train_loss) + "\t" + str(train_acc) + "\n")

    if not args.model_file == "":
        torch.save(model.state_dict(), args.model_file + "embedder_{}.pth".format(args.emb_dim))
